...
Invalidation: {BULL_SETUP['invalidation']}
...
1. HTF BIAS EVIDENCE
   - Is 1H/Daily structure bullish? Higher Highs and Higher Lows confirmed?
   ...
   - If ANY of these structural checks are bearish -> state NO LONG SETUP and stop.
...
2. PREMIUM / DISCOUNT CONFIRMATION
   ...
   - Note if price is in PREMIUM (above midnight open / above 50% of range). Premium = targets are nearby sell-side liquidity below. This is context for target selection, NOT a trade blocker. If daily bias is bullish and sweep + FVG confirm, a long in premium is valid at reduced conviction.
...
8. RISK CALCULATION
   ...
   - Stop loss: behind FVG candle 1 or OB body [exact level] (STRUCTURAL — not ATR)
   ...
   - Target 1: nearest BSL [exact level] — close 50% at T1. Move stop to breakeven after T1 hit.
...
12. TRADER CONVICTION ALIGNMENT (CONTEXT ONLY — does NOT reduce sizing)
   - If trader bias is BULLISH → note alignment. Reference: "Trader conviction aligns with bullish case"
   - If trader bias is BEARISH → note opposition. Acknowledge: "Note: trader entered
     session with bearish conviction." Present evidence — but do NOT reduce size for this.
